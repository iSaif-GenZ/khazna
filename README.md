...
| **Statistics Dashboard** | Category breakdown (pie chart) + 7-day income/expense trend (bar chart) using `fl_chart` |
...
| **Local Database** | `isar_community` (NoSQL, high performance) |
| **Charts** | `fl_chart` |
| **Image Handling** | `image_picker` + `image_cropper` |
| **Path & Storage** | `path_provider` |
| **Formatting** | `intl` |
...
|------|---------------|----------------------|
| Settings persistence | In-memory only | Integrate `shared_preferences` or Isar |
...
3. Ensure Isar code generation is up to date (`dart run build_runner build` if needed)
